...
/*
请根据每日 气温
列表，重新生成一个列表。对应位置的输出为：要想观测到更高的气温，至少需要等待的天数。如果气温在这之后都不会升高，请在该位置用
0 来代替。
*/
...
public:
  ...
  // 队列先进先出，s1放先入的元素，如果要先出，就要把s1的元素pop &
  // push进s2就行了
...
empty 这些操作是合法的。 你所使用的语言也许不支持队列。 你可以使用 list 或者
...
public:
  ...
  bool backTracking(vector<int> &groups, vector<int> &nums, int idx,
                    int target) {
    if (idx < 0)
      return true;
    int v = nums[idx--];
    for (int i = 0; i < groups.size(); i++) {
      if (groups[i] + v <= target) {
        groups[i] += v;
        if (backTracking(groups, nums, idx, target))
          return true;
        groups[i] -= v;
      }
      if (groups[i] == 0)
